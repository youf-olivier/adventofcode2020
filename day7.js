...
const objInput = input.reduce((acc, cur) => {
  const [name, cont] = cur.split(' bags contain ');
  return [...acc, [name, cont]];
}, []);

// Part 1

const getConteneurs = colorBag => objInput.filter(([, cont]) => cont.includes(colorBag));
const getBagConteneur = getConteneursResult => getConteneursResult.map(([a, b]) => a);
const getBagUseBag = bag => {
  const list = getBagConteneur(getConteneurs(bag));
  return [...list, ...new Set(list.reduce((acc, cur) => [...acc, ...getBagUseBag(cur)], []))];
};

// console.log(objInput);
// console.log(getConteneurs('shiny gold'));
// console.log(getBagConteneur(getConteneurs('shiny gold')));
// console.log(getBagConteneur(getConteneurs('dull chartreuse')));

console.log(getBagUseBag('shiny gold').length);

// Part 2
const getContenu = colorBag => objInput.find(([name]) => name === colorBag);

const getArrayBag = getConteneursResult => {
  if (getConteneursResult[1] === 'no other bags.') {
    return 0;
  }
  let totBag = 0;
  const listBag = [];
  getConteneursResult[1].split(',').forEach(sacstr => {
    totBag += sacstr.trim()[0] / 1;
    listBag.push([sacstr.trim()[0] / 1, sacstr.substring(2, sacstr.indexOf('bag')).trim()]);
  }, []);
  listBag.forEach(bag => {
    totBag += bag[0] * getArrayBag(getContenu(bag[1]));
  });
  return totBag;
};

console.log(getContenu('muted teal'));
console.log(getArrayBag(getContenu('shiny gold')));
